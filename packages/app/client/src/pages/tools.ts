export const TpWatermark = (CON: string) => {
  const H = 220; //  水印行高
  const W = 300; // => 水印宽度
  const R = -30; //  => 旋转度数（可为负值）
  const C = 'rgb(139,189,231)'; // => 水印字体颜色
  const S = 16; //  => 水印字体的大小
  const O = 0.25; // => 水印透明度（0~1之间取值）

  // 判断水印是否存在，如果存在，那么不执行
  if (document.getElementById('tp-watermark') != null) {
    return;
  }
  const TpLine = parseInt(String((document.body.clientWidth - 200) / W)) ; // 一行显示几列
  let StrLine = '';
  ...
  const TpColumn = parseInt(String(document.body.clientHeight / H)) ; // 一列显示几行
  let StrColumn = '';
  ...
  DivLayer.style.left = '160px'; // 改变整体水印的left值
